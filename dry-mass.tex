...
The moist mass-weighted velocity compenents, $\sum_\ell m^{(\ell)}_k \Delta \mathcal{P}^{(d)}_k u_k$ and $\sum_\ell m_k^{(\ell)} \Delta \mathcal{P}^{(d)}_k v_k$ respectively, are remapped seperately. Mapping the moist mass weighted velocity components conservatively leads to an angular momentum conserving vertical remapping algorithm. For the velocity components we do not enforce shape-preservation in the vertical remapping process.
