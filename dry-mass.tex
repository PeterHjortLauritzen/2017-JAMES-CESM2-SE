...
\end{equation}
From \eqref{eq:dalton} it is clear that in the presence of condensate one can not equate $p^{(d)}(z)$ with ${M}^{(d)}(z)$, nor $p^{(wv)}(z)$ with ${M}^{(wv)}(z)$. The partial pressures of dry air and water vapor are both affected by the weight of the condensates even though the condensates do not exert a gas pressure. Hence the partial pressure of dry air $p^{(d)}(z)$ at height $z$ is different from the hydrostatic dry air mass pressure $M^{(d)}(z)$; and similar for moisture. The hydrostatic balance for dry air mass, written in terms of differentials, is given by
\begin{equation}
...
dp^{(d)}(z)\ne -\rho_d g dz.
\end{equation}
The partial pressure of dry air $p^{(d)}(z)$ at height $z$ will increase in the presence of condensates whereas the weight of dry air does not. The differential of the moist pressure can be written in terms of the dry air mass hydrostatic pressure (under the hydrostatic assumption) though:
\begin{equation}
