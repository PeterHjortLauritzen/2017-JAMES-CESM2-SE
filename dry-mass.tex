...
The thermodynamic equations in CAM-SE are evolved using the 5-stage 3rd-order Runge-Kutta scheme described in \cite[][ see their equation (56)]{GU2016GMD}.  The stability of this class of time-stepping schemes is discussed in \citet[][ see their section 3.4]{DetAl2015GMD}.  For a given initial state vector at timestep $n$, $\vec{\psi}^{(0)} = \vec{\psi}^n$, the updated state vector $\vec{\psi}^{(5)} = \vec{\psi}^{n+1}$ is computed as follows:
\begin{align}
\vec{\psi}^{(1)} &= \vec{\psi}^{(0)} + \tfrac{\Delta t}{5} \vec{f}(\vec{\psi}^{(0)}), \nonumber \\
\vec{\psi}^{(2)} &= \vec{\psi}^{(0)} + \tfrac{\Delta t}{5} \vec{f}(\vec{\psi}^{(1)}), \nonumber \\
\vec{\psi}^{(3)} &= \vec{\psi}^{(0)} + \tfrac{\Delta t}{3} \vec{f}(\vec{\psi}^{(2)}), \\
\vec{\psi}^{(4)} &= \vec{\psi}^{(0)} + \tfrac{2 \Delta t}{3} \vec{f}(\vec{\psi}^{(3)}), \nonumber \\
\vec{\psi}^{(5)} &= -\tfrac{1}{4} \vec{\psi}^{(0)} + \tfrac{5}{4} \vec{\psi}^{(1)} + \tfrac{3 \Delta t}{4} \vec{f}(\vec{\psi}^{(4)}), \nonumber
\end{align} where $\vec{f}(\vec{\psi})$ denotes the discrete right-hand-side terms {\color{red}(??)}.  The resulting method is linearly and non-linearly third-order accurate.  The scheme possesses a stability region which is provably optimal in terms of its extent along the imaginary axis among all 5-stage 3rd-order Runge-Kutta schemes, $[-i\sqrt{15}, i\sqrt{15}]$.  Since the largest eigenvalue of the 1D 4th-order spectral element spatial discretization ($N_p = 4$) is $i \sqrt{10/3}$, the resulting scheme satisfies a Courant-Friedrichs-Lewy condition given by
\begin{align}
