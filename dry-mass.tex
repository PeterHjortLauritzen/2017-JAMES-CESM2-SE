...
\authors{P.H. Lauritzen\affil{1}\thanks{1850 Table Mesa Drive, Boulder, Colodaro, USA}, R.D. Nair\affil{1}, A.R. Herrington\affil{2},P. Callaghan\affil{1}, S. Goldhaber\affil{1}, J.M. Dennis\affil{1}, J.T. Bacmeister\affil{1}, B.E. Eaton\affil{1}, C.M. Zarzycki\affil{1}, Mark A. Taylor\affil{2}, P.A. Ullrich\affil{3}, T. Dubos\affil{4}, A. Gettelman\affil{1},  R.B. Neale\affil{1}, B. Dobbins\affil{1}, K.A. Reed\affil{5}, C. Hannay\affil{1} and J.J. Tribbia\affil{1}}

 \affiliation{1}{National Center for Atmospheric Research, Boulder, Colorado, USA}
 \affiliation{2}{Sandia National Laboratories, Albuquerque, New Mexico, USA}
 \affiliation{3}{Department of Land, Air and Water Resources, University of California, Davis, California, USA}
 \affiliation{4}{Ecole Polytechnique, UMR 8539, Laboratoire de M\'et\'eorologie Dynamique/IPSL, Palaiseau, France}
 \affiliation{5}{School of Marine and Atmospheric Sciences, Stony Brook University, State University of New York, Stony Brook, New York}
...
\end{equation}
Henceforth we assume that the volume of condensates is negligible compared to the gaseous components of air.
...
\end{equation}
where $\delta Q$ is the amount of heat per unit mass that is supplied reversibly to the moist air, and $s$ is specific entropy of moist air. Introducing specific internal energy $e$ and specific enthalphy $h=e+p\alpha$, the right-hand side of \eqref{eq:slt} can be written as
\begin{align}
...
\end{equation}
With the above definition of mass-weighted heat capacity and gas constant, the thermodynamic equation can be written as
\begin{equation}
\delta T-\frac{1}{\rho\, c_p}\delta p=\frac{1}{c_p}\delta Q.\label{eq:thermo2}
\end{equation}
The ideal gas law can be written as \citep[which is consistent with equation 7.1 in ][]{B2003JAS}
\begin{equation}
...
In a typical CAM setup where the model top is approximately 40km, the maximum stable time-step for solving the continuity equation for water species $\ell$, $\ell \in \mathcal{L}_{water}$ (referred to as tracer advection), is limited by the maximum hoirzontal advective wind speed. The maximum stable time-step for the remaining equations of motion (thermodynamic equation, momentum equations and the continuity equation for dry air) is limited by the fastest gravity waves.  Since the maximum advective winds are typically slower than the fastest gravity waves, different time-stepping methods are used for tracer advection and the remaining equations of motion. While the overall time-step is the same, different Runge-Kutta (RK) methods are used to maintain stability. 
...
The momentum, thermodynamic and dry air continuity equations are solved using an explicit nonlinearly third-order accurate five-stage Runga-Kutta method based on \cite{KG1984MCSb} and described in \cite[][ see their equation (56)]{GU2016GMD}. We will refer to this scheme as KG53. The stability of this class of time-stepping schemes is discussed in \citet[][ see their section 3.4]{DetAl2015GMD} but it is notable that the KG methods can be proven to incorporate the greatest possible extent of the imaginary axis for a given number of stages and order-of-accuracy.. The inviscid equations of motion are advanced one dynamics time-step using the KG53 method followed by an application of the hyperviscosity operators through solving the advection-diffusion equation of the updated prognostic variables. Note that it is necessary to subcycle the hyperviscosity-step for stability. The viscosity terms are computed as described in section \ref{sec:hyper} and discretized using the SE method presented in Appendix \ref{sec:dissipation}.
...
%{\color{red}{[Adam: Thanks Colin, I've tried to clarify this paragraph a little bit more, through referencing th schematic more directly, among other things. I have clarified the schematic per your suggestions.]}}
