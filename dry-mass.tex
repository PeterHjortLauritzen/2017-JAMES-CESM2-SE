...
\subsubsection{Dry air partial pressure and pressure exerted by the weight of dry air}\label{sec:dryairp}
The observant reader will have noticed that we denote the pressure exerted by the dry air $M^{(d)}$ and not the dry air partial pressure $p^{(d)}$. As explained below this is because $M^{(d)}\neq p^{(d)}$ when air is a multi-phase fluid (i.e. in the presence of condensates). 
...
\end{equation}
From \eqref{eq:dalton} it is clear that in the presence of condensate one can not equate $p^{(d)}(z)$ with ${M}^{(d)}(z)$, nor $p^{(wv)}(z)$ with ${M}^{(wv)}(z)$. The partial pressures of dry air and water vapor are both affected by the weight of the condensates even though the condensates do not exert a pressure. Hence the partial pressure of dry air $p^{(d)}(z)$ at height $z$ is different from the pressure exerted by the weight of dry air above $M^{(d)}(z)$; and similar for moisture. The hydrostatic balance for dry air, written in terms of differentials, is given by
\begin{equation}
...
dp^{(d)}(z)\ne -\rho_d g dz.
\end{equation}
The partial pressure of dry air $p^{(d)}(z)$ at height $z$ will increase in the presence of condensates whereas the weight of dry air does not. The differential of the moist pressure can be written in terms of the pressure exerted by the weight of dry air mass (under the hydrostatic assumption) though:
\begin{equation}
...
\end{equation}
In all, $M^{(d)}(z)$ refers to dry air partial pressure at height $z$ if there were no condensates present (i.e. air is a gas only) and similarly for water vapor. Henceforth we drop the notation of the vertical dependence (in this section we used $z$) on $M^{(\ell)}(z)=M^{(\ell)}$. Since $\eta^{(d)}$ is the vertical coordinate $M^{(\ell)}$ will henceforth refer to the pressure exerted by the weight of water form $\ell$ above $\eta^{(d)}$.
%p_s=p^{(d)}+p^{(wv)}=\int
...
\end{equation}
so that the moist pressure (in the absence of condensates) at half levels can be computed as the sum of the pressure exerted by the weight of dry air and water vapor
\begin{equation}
...
\end{equation}
Then use the resulting change in the pressure exerted by the dry mass at the surface to determine the reference values for dry-mass thicknesses
\begin{equation}
