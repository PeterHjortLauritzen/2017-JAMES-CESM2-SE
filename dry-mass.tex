...
\authors{P.H. Lauritzen\affil{1}\thanks{1850 Table Mesa Drive, Boulder, Colodaro, USA}, R.D. Nair\affil{1}, A.R. Herrington\affil{2},P. Callaghan\affil{1}, S. Goldhaber\affil{1}, J.M. Dennis\affil{1}, J.T. Bacmeister\affil{1}, B. Eaton\affil{1}, C.M. Zarzycki\affil{1}, Mark A. Taylor\affil{3}, A. Gettelman\affil{1},  R.B. Neale\affil{1}, B. Dobbins\affil{1}, K.A. Reed\affil{2}, T. Dubos\affil{4}, C. Hannay\affil{1} and J.J. Tribbia\affil{1}}
...
\end{equation}
where $p^{(d)}$ is the partial pressure exterted by dry air (the pressure dry air would exert if it alone would occupy the volume) and $p^{(wv)}$ is the partial pressure of water vapor (again, the pressure exerted by water vapor if it alone would occupy the volume). Note that the condensates do not exert a pressure (more in this in Section \ref{sec:dryairp}). We may write \eqref{eq:ig} in terms of $\rho^{(gas)}$ by multiplying \eqref{eq:ig} with $\frac{\rho^{(gas)}}{\rho^{(gas)}}$ and write the equation in terms of mixing ratios
\begin{equation}
...
Let $\frac{1}{g}M^{(d)}_s$ be the mass of dry air per unit area at the surface ($M^{(d)}_s$ is the hydrostatic dry air mass pressure at the surface) and $\frac{1}{g}M_t^{(d)}$ the weight of air per unit area in the column above the model top. We assume that there is no moisture or condensate above the model top so $p_t=M_t^{(d)}$. Consider a general, terrain-following, vertical coordinate $\eta^{(d)}$ that is a function of the weight of dry air $\frac{1}{g}M^{(d)}$ 
\begin{equation}
\eta^{(d)}=h(M^{(d)},M_s^{(d)}),
\end{equation}
...
\subsubsection{Dry air partial pressure and hydrostatic dry air mass pressure}\label{sec:dryairp}
The observant reader will have noticed that we denote the hydrostatic dry air mass pressure $M^{(d)}$ and not the dry air partial pressure $p^{(d)}$. As explained below this is because $M^{(d)}\neq p^{(d)}$ when air is a multi-phase fluid (i.e. in the presence of condensates). 

The hydrostatic moist pressure at given height $z$ can be computed from the hydrostatic balance
\begin{align}
p(z)&=\int^{z'=\infty}_{z'=z}\rho\, g\, dz',\\
 &=\int^{z'=\infty}_{z'=z}\rho_d \left( \sum_{\ell \in \mathcal{L}_{all}} m_\ell \right)\, g\, dz',\\
 &=\sum_{\ell \in \mathcal{L}_{all}} M^{(\ell)}(z),\label{eq:z'2}
\end{align}
where the right-hand side of \eqref{eq:z'2} is the weight of dry air, water vapor, cloud liquid, cloud ice, rain and snow per unit area above height $z$, respectively:
\begin{equation}
{M}^{(\ell)}(z)=\int^{z'=\infty}_{z'=z}\rho_d \, m_\ell \, g\, dz'\label{eq:z'}.
\end{equation}
...
\begin{equation}
p^{(d)}(z)+p^{(wv)}(z)=\sum_{\ell \in \mathcal{L}_{all}} {M}^{(\ell)}(z).\label{eq:dalton}
\end{equation}
From \eqref{eq:dalton} it is clear that in the presence of condensate one can not equate $p^{(d)}(z)$ with ${M}^{(d)}(z)$, nor $p^{(wv)}(z)$ with ${M}^{(wv)}(z)$. The partial pressures of dry air and water vapor are both affected by the weight of the condensates even though the condensates do not exert a pressure. Hence the partial pressure of dry air $p^{(d)}(z)$ at height $z$ is different from the hydrostatic dry air mass pressure $M^{(d)}(z)$; and similar for moisture. The hydrostatic balance for dry air mass, written in terms of differentials, is given by
\begin{equation}
dM^{(d)}(z)=-\rho_d g dz,\label{eq:dry_atm_hydro}
\end{equation}
whereas, in a moist atmosphere (with condensates), a dry air partial pressure hydrostatic equation does not hold
\begin{equation}
dp^{(d)}(z)\ne -\rho_d g dz.
\end{equation}
The partial pressure of dry air $p^{(d)}(z)$ at height $z$ will increase in the presence of condensates whereas the weight of dry air does not. The differential of the moist pressure can be written in terms of the dry air mass hdyrostatic pressure (under the hydrostatic assumption) though:
\begin{equation}
dp(z)=-\rho g \, dz = -\rho_d \left( \sum_{\ell \in \mathcal{L}_{all}} m_\ell\right)g\, dz = dM^{(d)}(z)\left( \sum_{\ell \in \mathcal{L}_{all}} m_\ell\right).
\end{equation}
In all, $M^{(d)}(z)$ refers to dry air partial pressure at height $z$ if there were no condensates present (i.e. air is a gas only) and similarly for water vapor. Henceforth we drop the notation of the vertical dependence (in this section we used $z$) on $M^{(\ell)}(z)=M^{(\ell)}$. Since $\eta^{(d)}$ is the vertical coordinate $M^{(\ell)}$ will henceforth refer to hydrostatic pressure for water form $\ell$   at $\eta^{(d)}$. The hydrostatic dry-mass pressure is not a directly measurable quantity but a theoretical construction for the dry-mass vertical coordinate. Only moist pressure is directly measurable.
%p_s=p^{(d)}+p^{(wv)}=\int
...
In the vertical the atmosphere is discretized into $nlev$ floating Lagrangian layers. The vertical index is 1 for the upper most level and $nlev$ in the lower most level. The level interfaces are referred to as half-levels so that layer k is bounded by interface level $k+1/2$ and $k-1/2$. Since we are using a dry-mass vertical reference coordinate, the dry air mass hydrostatic pressure at the layer interfaces is defined in terms of the hybrid coefficients $A$ and $B$
...
\end{equation}
so that the moist pressure (in the absence of condensates) at half levels can be computed as the sum of the hydrostatic dry air and moisture mass pressure
\begin{equation}
...
\end{align}
where $\langle M^{(d)} \rangle_{1/2}=p^{(d)}_t$ and  $\langle M^{(d)} \rangle_{nlev+1/2}=\langle M^{(d)}_s \rangle$. At this point, we have a set of self-consistent element average values which satisfy the hydrostatic equation
\begin{equation}
...
\end{equation}
Then use the resulting change in hydrostatic dry air mass pressure at the surface to determine the reference values for dry-mass thicknesses
\begin{equation}
