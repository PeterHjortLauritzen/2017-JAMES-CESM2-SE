...
Let $\frac{1}{g}M^{(d)}_s$ be the mass of a column of dry air per unit area at the surface ($M^{(d)}_s$ is the hydrostatic dry air mass pressure at the surface) and $\frac{1}{g}M_t^{(d)}$ the mass of air per unit area in the column above the model top. We assume that there is no moisture or condensate above the model top so $p_t=M_t^{(d)}$. Consider a general, terrain-following, vertical coordinate $\eta^{(d)}$ that is a function of the hydrostatic dry mass pressure $M^{(d)}$ 
\begin{equation}
\eta^{(d)}=h(M^{(d)},M_s^{(d)}),
\end{equation}
