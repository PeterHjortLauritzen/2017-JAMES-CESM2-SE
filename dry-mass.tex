...
\end{equation}
where $R^{(wv)}\equiv \frac{N_A}{\mathcal{M}^{(wv)}}$ is the gas constant for water vapor. According to Dalton's law of partial pressures, the total pressure exerted is equal to the sum of the partial pressures of the individual gases:
\begin{equation}
\label{eq:ptmp}
...
\end{equation}
the right-hand side of \eqref{eq:slt} can be written as
\begin{align}
...
\begin{equation}
\frac{RT}{p}=\frac{V^{(gas)}}{\sum m^{(\ell)}}
\end{equation}
...
and
\begin{equation}
p=\rho R T,\text{ if }V^{(cond)=0},\label{eq:ig4}
\end{equation}
since
\begin{equation}
\rho \equiv \frac{1}{V}\sum_{\ell \in \mathcal{L}_{all}}m^{(\ell)}.
\end{equation}
