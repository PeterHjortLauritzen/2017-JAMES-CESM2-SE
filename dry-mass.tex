...
As for $M^{(d)}$, the weight of water vapor $\frac{1}{g}M^{(wv)}$ per unit area can be written as
\begin{equation}
\frac{1}{g}{M}^{(wv)}(\varphi,z)=-\int_z^{z_t}\frac{p(\varphi,z)}{R^{(d)} T_v(\varphi,z)}q^{(wv)}(\varphi,z)\, dz,\label{eq:Upvw}
\end{equation}
so that the moist pressure at half levels can be computed as the sum of the hydrostatic dry air and moisture mass pressure since there are no condensates present in the initial condition
\begin{equation}
