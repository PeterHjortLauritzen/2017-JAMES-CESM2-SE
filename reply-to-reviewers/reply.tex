...
That is indeed confusing language used in section 3.4. For the frictionless equations of motion (i.e. no hyperviscosity terms) the SE discretization is energy conserving to time-truncation errors. The hyper-viscosity on momentum is converted into heating so that energy is conserved with respect to momentum diffusion. Energy conservation is violated by hyperviscosity on temperature and pressure (a detailed energy analysis of CAM-SE is the subject of a manuscript currently in preparation). The damping applied to mass is mass-conserving to computational rounding error.\\

Changes to manuscript: section 3.4 has been reworded.
...
\begin{itemize}
\item {\color{blue}{J.-F.: you should rewrite the title, maybe something like NCAR release of CAM-SE in CESM2.0}}
\end{itemize}
